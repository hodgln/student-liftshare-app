...
const PassengerCard = (props) => {
    ...
    const getMapImg = async (urlpath) => {
        try {
            ...
            const response = await fetch("http://localhost:8081/locations/signurl", {
                method: 'POST',
                headers: myHeaders,
    ...
    const getPolyline = async (origin, destination) => {
        try {
            ...
            return (
                `https://maps.googleapis.com/maps/api/staticmap?&size=400x400&path=weight:3%7Ccolor:0x0352A0CC%7Cenc:${parseRes.route}&key=AIzaSyAABf0pH3xmQE_riwJXkrazEQADU0fqEss&map_id=76404385f094ea10`
            )
